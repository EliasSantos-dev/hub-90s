# Sub-A: Engine Expansion — Dive Attacks, Oscillation, Particles

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Expandir engine.ts com dive attacks (Bézier), oscilação suave da formação, partículas, screen shake e parallax stars — sem quebrar os 65 testes existentes.

**Architecture:** Adiciona novos tipos (DivingEnemy, Particle, Star) e campos ao GameState. Mantém SCORE_BY_ROW={0:30,1:20,2:10} e Enemy.x/y intactos para compatibilidade com testes. Formation movement migra de step-based para velocity-based contínuo.
...
Os testes verificam:
- `SCORE_BY_ROW[0] === 30`, `[1] === 20`, `[2] === 10` — NÃO alterar
- `firstEnemy.x` e `firstEnemy.y` existem no tipo Enemy — NÃO remover
- `state.enemies`, `state.bullets`, `state.player`, `state.lives`, `state.score`, `state.gameStatus` — manter

---

## Task 1: Substituir engine.ts
...
Escrever o seguinte conteúdo em `src/lib/game/engine.ts`:
...
```bash
cd /home/elias-santos/repos/game-90s && npm test 2>&1 | tail -12
```
Expected: `Tests  65 passed (65)`.  
Se algum teste falhar — provavelmente é SCORE_BY_ROW ou tipo Enemy — leia a mensagem de erro e corrija sem mudar os valores 30/20/10.
...
git commit -m "feat(engine): dive attacks bézier, oscillation suave, particles, screen shake, parallax stars"
```
