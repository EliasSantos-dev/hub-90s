# Sub-B: Renderer — Pixel Art, Parallax, Partículas, Shake

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Substituir renderer.ts por versão completa com pixel art do hambúrguer (spec seção 4), sprites animados de inimigos (bee/butterfly), parallax stars, partículas, screen shake e rendering de diving enemies.

**Architecture:** O arquivo `src/lib/game/engine.ts` já foi expandido e exporta os tipos `DivingEnemy`, `Particle`, `Star`, e o `GameState` já possui `divingEnemies`, `particles`, `stars`, `animFrame`, `shakeTimer`, `shakeIntensity`. O renderer só usa esses campos — sem mudanças de lógica.

**Tech Stack:** TypeScript, Canvas 2D

---

## Task 1: Substituir renderer.ts
...
Escrever o seguinte conteúdo em `src/lib/game/renderer.ts`:
...
git commit -m "feat(renderer): pixel art hambúrguer, bee/butterfly animados, parallax, partículas, screen shake"
```
